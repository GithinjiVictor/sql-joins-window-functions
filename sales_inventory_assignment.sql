...
-- 10. Write a query to count the number of sales for each product from the `Sales` table.       --CHECK THROUGH--
...
select product_id,sum(quantity_sold) as total_quantity_sales from sales group by product_id ;
...
select count(s.customer_id) as Gold_Customer_Sales
from customers c 
...
select c.first_name,c.last_name,SUM(s.quantity_sold)
from customers c 
join sales s on c.customer_id =s.customer_id 
...
select count(s.quantity_sold) 
from sales s 
where extract(year from s.sale_date)=2023 and extract(month from sale_date)=12;
-- 32. Write a query to find the total amount spent by each customer in 2023 and list the customers in descending order.
select c.first_name,c.last_name, sum(s.total_amount)
from sales s 
join customers c on s.customer_id =c.customer_id 
where extract(year from s.sale_date )=2023
group by c.first_name ,c.last_name 
...
select p.product_name,s.sale_id,i.stock_quantity
from products p 
join sales s on p.product_id =s.product_id 
...
select c.first_name ,c.last_name ,count(s.sale_id)as purchase_count
from customers c 
join sales s on c.customer_id =s.customer_id 
group by c.first_name ,c.last_name 
...
select c.first_name,count(s.quantity_sold ) as products_purchased
from customers c 
join sales s on c.customer_id =s.customer_id 
...
(select p.product_name,i.stock_quantity as stock_status
from products p 
join inventory i on p.product_id =i.product_id 
group by p.product_name,stock_status   
order by i.stock_quantity desc limit 1)
union
(select p.product_name,i.stock_quantity as stock_status
from products p 
join inventory i on p.product_id =i.product_id 
group by p.product_name,stock_status
order by i.stock_quantity asc limit 1);
...
select p.product_name,c.first_name,c.last_name,sum(s.total_amount)
from customers c
...
select p.product_name,extract(month from s.sale_date) as sale_month,extract (year from s.sale_date)as sale_year,sum(s.total_amount) as total_sales
from products p
join sales s on p.product_id=s.product_id
